...
def batchProcess(data, loop, batchSize, intervals, bandwidth):
    ...
    for i in range(batchSize):
        arldetect = np.zeros(len(etaValues))
        error = []
        error2 = []
        print(i)
                    ...
                    if l >= r:
                        ...
                        if newStopRule > maxStopRule:
                            ...
                            for idx, eta in enumerate(etaValues):
                                ...
                                if newStopRule > eta and arldetect[idx] == 0.0:
                                    badCount[idx] += 1
    ...
    return test49, stopRule, sig1, sig2, errors, test491, DeltaBTC[24], HVals[25], sigVals[25], test261, test262, np.mean(errors2)
...
data, stopRule, error, error2 = MCSims(1000, 1000, 30000, 12345, bandwidth)
